...
//    func fetchSongRecommendationForMood(minValence: Float, maxValence: Float, minEnergy: Float, maxEnergy: Float, completion: @escaping (Result<(Track, UIImage?), Error>) -> Void) {
//        guard let accessToken = self.appRemote.connectionParameters.accessToken else {
//            completion(.failure(NSError(domain: "SpotifyService", code: 0, userInfo: [NSLocalizedDescriptionKey: "Access token is nil"])))
//            return
//        }
...
//        var components = URLComponents(url: url, resolvingAgainstBaseURL: false)!
//        components.queryItems = [
//            URLQueryItem(name: "limit", value: "1"),  // Fetching only one track
//            URLQueryItem(name: "seed_genres", value: "latin"), // Example genre
//            URLQueryItem(name: "min_valence", value: String(minValence)),
//            URLQueryItem(name: "max_valence", value: String(maxValence)),
//            URLQueryItem(name: "min_energy", value: String(minEnergy)),
...
//                let decoder = JSONDecoder()
//                let recommendations = try decoder.decode(RecommendationResponse.self, from: data)
//                guard let firstTrack = recommendations.tracks.first else {
//                    completion(.failure(NSError(domain: "SpotifyService", code: 2, userInfo: [NSLocalizedDescriptionKey: "No tracks found"])))
//                    return
//                }
//
//                // Fetch the image for the single track
//                guard let imageUrl = firstTrack.album.images.first?.url else {
//                    completion(.success((firstTrack, nil))) // No image available
//                    return
//                }
//
//                URLSession.shared.dataTask(with: imageUrl, completionHandler: { imageData, _, imageError in
//                    guard let imageData = imageData, imageError == nil, let image = UIImage(data: imageData) else {
//                        completion(.success((firstTrack, nil))) // Fail to load image
//                        return
//                    }
//                    completion(.success((firstTrack, image)))
//                }).resume()
//            } catch {
//                completion(.failure(error))
//            }
